...
LABELS = {
    # github defaults
    "bug": ("d73a4a", "Something isn't working",),  # red
    "duplicate": ("cfd3d7", "This issue or pull request already exists",),  # grey
    "enhancement": ("a2eeef", "New feature or request",),  # light blue
    "good first issue": ("7057ff", "Good for newcomers",),  # blue/purple-ish
    "help wanted": ("008672", "Extra attention is needed",),  # teal
    "invalid": ("e4e669", "This doesn't seem right",),  # yellow
    "question": ("d876e3", "Further information is requested"),  # purple
    "wontfix": ("ffffff", "This will not be worked on",),  # white
    # custom
    "wip": ("fbca04", "Work in progress",),  # yellow
    "discussion": ("d876e3", "Needs to be discussed"),  # light purple
    "documentation": (
        "bfdadc",  # light teal
    ...
    for repo in repos.values():
        ...
        for label in repo.labels():
            ...
            if label.color.lower() != lcolor:
                print(
                    "updating %r color from #%s to #%s in %r"
            ...
            if label.description is not None and label.description != ldescription:
                print(
                    "updating %r description from %r to %r in %r"
